...
/**
 * Concept-to-path map: answer questions like "where does login happen?"
 * before someone has to ask. Pure heuristics over file paths and content.
 *
 * Robustness rules:
 *   1. Path patterns match path SEGMENTS only, never accidental substrings
 *      (so `/policy/` does not match `privacyPolicy.tsx`).
 *   2. Content patterns are anchored with word boundaries.
 *   3. Content matches are gated by minimum strong signals (≥2 distinct
 *      patterns OR ≥3 files) to suppress one-off coincidental string hits.
 *   4. Files inside `scripts/`, `migrations/`, etc. are de-prioritised.
 *   5. Concepts can declare `requiredKind` to restrict where they apply
 *      (e.g. "Database schemas" only fires for backend projects).
 */

interface ConceptDef {
  concept: string;
  description: string;
  // file-path patterns and content patterns that match this concept
  pathPatterns: RegExp[];
  contentPatterns?: RegExp[];
  // Concept only applies to these project kinds (any if undefined)
  requiredKind?: ProjectKind[];
  // Minimum content-match files required when path matching fails
...
// `privacyPolicy.tsx`.
function pathSegmentTest(fp: string, re: RegExp): boolean {
  ...
  if (re.test(fp)) {
    ...
    if (reSrc.includes('\\/') || reSrc.includes('[\\\\/]')) return true;
...
export function buildWhereToLook(input: {
  files: Map<string, FileNode>;
  contentMap: Map<string, string>;
  allFilePaths: string[];
  projectKind?: ProjectKind;
}): ConceptLocation[] {
  const out: ConceptLocation[] = [];
  ...
  for (const def of CONCEPTS) {
    ...
    for (const fp of input.allFilePaths) {
      if (def.pathPatterns.some((re) => pathSegmentTest(fp, re))) matches.add(fp);
    }
    ...
    if (def.contentPatterns && matches.size === 0) {
      ...
      if (candidate.size >= need || distinctPatterns.size >= 2) {
        for (const fp of candidate) matches.add(fp);
      }
    }
...
// canonical example).
function stripStringAndRegexLiterals(src: string): string {
...
const RISK_HEURISTICS: Array<{ pattern: RegExp; risk: string }> = [
...
function inferRisk(name: string): string {
...
export function buildEnvMaturity(input: {
  envContracts: ImplicitContract[];
  contentMap: Map<string, string>;
}): EnvVarMaturity[] {
  const out: EnvVarMaturity[] = [];
