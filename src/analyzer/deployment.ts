...
function parseDockerfile(filePath: string, content: string): DeploymentInfo['dockerfiles'][0] {
...
function parseComposeFile(content: string): DeploymentInfo['composeServices'] {
  const services: DeploymentInfo['composeServices'] = [];
  const lines = content.split('\n');

  let inServices = false;
  let currentService: string | null = null;
  let currentIndent = 0;
  let serviceData: Partial<DeploymentInfo['composeServices'][0]> = {};
  let currentKey = '';
  ...
  for (const line of lines) {
    ...
    if (listItem && currentService) {
      ...
      if (currentKey === 'ports') {
        if (!serviceData.ports) serviceData.ports = [];
        serviceData.ports.push(val);
      } else if (currentKey === 'depends_on') {
        if (!serviceData.dependsOn) serviceData.dependsOn = [];
        serviceData.dependsOn.push(val.replace(/:$/, ''));
      } else if (currentKey === 'volumes') {
        if (!serviceData.volumes) serviceData.volumes = [];
...
function parseCiFile(filePath: string, content: string): DeploymentInfo['ciPipelines'][0] {
  let tool = 'Unknown CI';
  if (filePath.includes('.github/workflows')) tool = 'GitHub Actions';
  else if (filePath.includes('.gitlab-ci')) tool = 'GitLab CI';
  else if (filePath.includes('.circleci')) tool = 'CircleCI';
  else if (filePath.includes('Jenkinsfile')) tool = 'Jenkins';
  else if (filePath.includes('.travis')) tool = 'Travis CI';
  else if (filePath.includes('bitbucket')) tool = 'Bitbucket Pipelines';
  else if (filePath.includes('azure')) tool = 'Azure DevOps';
  ...
  for (const line of lines) {
    ...
    if (trimmed === 'on:') { inOn = true; inJobs = false; continue; }
    if (trimmed === 'jobs:') { inJobs = true; inOn = false; continue; }
    ...
    // GitLab CI stages
    if (trimmed.startsWith('stages:')) continue;
...
function parseK8sFile(filePath: string, content: string): DeploymentInfo['k8sResources'] {
  const resources: DeploymentInfo['k8sResources'] = [];
  ...
  for (const doc of docs) {
    ...
    for (const line of doc.split('\n')) {
      ...
      if (inMetadata && line.search(/\S/) <= 0) inMetadata = false;
    }
