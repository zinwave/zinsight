...
exports.buildHappyPath = buildHappyPath;
...
/* ----------------------------- pickers --------------------------------- */
function pickRepresentativeRoute(routes, files) {
...
/* ---------------------------- detectors -------------------------------- */
function detectMiddleware(asts) {
    const out = [];
    for (const [fp, ast] of asts) {
        if (!ast)
            continue;
        if (!/middleware/i.test(fp) && !/main\.(t|j)sx?$/.test(fp))
            continue;
        let foundUse = false;
...
function guessServiceFile(controllerFile, files) {
...
function detectsExternalCallInFile(file, asts) {
    if (!file)
        return false;
    const ast = asts.get(file);
    if (!ast)
