...
function analyze(rootDir, onProgress, options = {}) {
    ...
    const filePaths = (0, file_discovery_1.discoverFiles)(resolvedRoot);
    ...
    const techStack = (0, tech_stack_1.detectTechStack)(rootDir, filePaths, actualDbTypes, project.language);
...
function readProjectInfo(rootDir, invokedFrom, language, options) {
...
function hasAnalyzableCode(dir) {
    try {
...
function detectLanguage(rootDir) {
    ...
    if (phpDeps > 0 && jsTsDeps === 0)
        return 'php';
    if (phpDeps > 0 && phpDeps >= jsTsDeps * 2)
        return 'php';
    if (has('go.mod'))
        return 'go';
    if (has('Cargo.toml'))
...
function detectProjectKind(input) {
    ...
    if (isCli && !hasFrontend && !hasBackend)
        return 'cli';
    ...
    if (!hasFrontend && !hasBackend && files.length > 0) {
        ...
        if (hasIndexHtml && !files.some((f) => /\.(tsx|jsx)$/.test(f)))
            return 'static-site';
        return 'library';
    }
    return 'unknown';
...
function computeCapabilities(input) {
    ...
    const hasAuth = routes.some((r) => r.guards && r.guards.length > 0) ||
        [...input.files.keys()].some((f) => /[\\/]auth[\\/]/.test(f) || /\.auth\.(t|j)s/.test(f) || /[\\/]middleware[\\/].*auth/i.test(f));
    const hasAuthorization = routes.some((r) => r.guards && r.guards.some((g) => /role|permission|rbac|policy/i.test(g))) ||
    ...
    let hasAi = false;
    for (const [, content] of contentMap) {
        if (/from\s+['"](openai|@anthropic-ai\/sdk|@google\/generative-ai|@google\/genai|cohere-ai|@mistralai\/[^'"]+|groq-sdk|together-ai|@huggingface\/inference|ollama)['"]/.test(content) ||
    ...
    for (const [, content] of contentMap) {
        if (/from\s+['"](bull|bullmq|kafkajs|amqplib|@aws-sdk\/client-sqs|kue|agenda)['"]/.test(content) ||
    ...
    for (const [, content] of contentMap) {
        if (/from\s+['"](nodemailer|@sendgrid\/[^'"]+|mailgun(\.js)?|postmark|@aws-sdk\/client-ses)['"]/.test(content) ||
    ...
    for (const [, content] of contentMap) {
        if (/from\s+['"](ioredis|redis|@nestjs\/cache-manager|cache-manager|lru-cache)['"]/.test(content) ||
