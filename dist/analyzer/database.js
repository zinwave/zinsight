...
const modelFieldsMap = new Map(); // className → Set of field names ending in Id
// Collected full schema data per model (all fields with types)
const modelSchemasMap = new Map();
// Collected table relationships from JOINs
const joinRelationships = [];
function detectDatabases(files, packageDeps) {
    ...
    for (const [type, data] of dbMap) {
        ...
        if (data.files.size === 0 && !data.importedInCode)
            continue;
        ...
        for (const jr of joinRelationships) {
            if (modelSet.has(jr.from.toLowerCase()) && modelSet.has(jr.to.toLowerCase())) {
                const exists = relationships.some(r => r.from.toLowerCase() === jr.from.toLowerCase() && r.to.toLowerCase() === jr.to.toLowerCase());
                if (!exists)
                    relationships.push(jr);
            }
        }
...
function deduplicateModels(models) {
    ...
    const seen = new Map(); // normalized → best form
...
function inferRelationships(models, fieldData) {
    const relationships = [];
...
function scanFileForDb(filePath, ast, dbMap) {
    (0, traverse_1.default)(ast, {
        ...
        CallExpression({ node }) {
            ...
            if (node.callee.type === 'MemberExpression' && node.callee.property.type === 'Identifier') {
                ...
                if (method === 'define' && node.arguments.length >= 1 && node.arguments[0].type === 'StringLiteral') {
                    for (const sqlType of ['postgresql', 'mysql']) {
        ...
        ObjectProperty({ node, parent }) {
            ...
            if (node.key.type === 'Identifier' && node.key.name === 'TableName' &&
            ...
            if (node.key.type === 'Identifier' && node.key.name === 'AttributeDefinitions' &&
                ...
                const typeMap = { S: 'string', N: 'number', B: 'Buffer', BOOL: 'boolean' };
                for (const elem of node.value.elements) {
                    ...
                    for (const p of elem.properties) {
                        if (p.type !== 'ObjectProperty' || p.key.type !== 'Identifier')
                            continue;
                        if (p.key.name === 'AttributeName' && p.value.type === 'StringLiteral')
                            attrName = p.value.value;
                        if (p.key.name === 'AttributeType' && p.value.type === 'StringLiteral')
                            attrType = typeMap[p.value.value] || p.value.value;
                    }
                    if (attrName)
                        fields.push({ name: attrName, type: attrType, required: true, isIndex: true });
                }
        ...
        StringLiteral({ node }) {
            ...
            if (val.length < 10 || val.length > 5000)
                return;
            ...
            const sqlType = dbMap.has('postgresql') ? 'postgresql' :
 ...
 */
function extractFieldInfo(fieldName, member) {
    const field = { name: fieldName, type: 'unknown' };
                    ...
                    for (const prop of arg.properties) {
                        ...
                        if (propName === 'default') {
                            if (prop.value.type === 'NullLiteral')
                                ...
                                field.defaultValue = String(prop.value.value);
                            else if (prop.value.type === 'NumericLiteral')
                                field.defaultValue = String(prop.value.value);
                            else if (prop.value.type === 'StringLiteral')
                                field.defaultValue = `"${prop.value.value}"`;
...
function resolveTypeAnnotation(node) {
    ...
    switch (node.type) {
        ...
        case 'TSUnionType':
            ...
            const nonNull = node.types?.filter((t) => t.type !== 'TSNullKeyword' && t.type !== 'TSUndefinedKeyword');
            if (nonNull?.length === 1)
 ...
 */
function extractCreateTableSchema(sql, filePath) {
    ...
    while ((match = createPattern.exec(sql)) !== null) {
        ...
        if (!tableName || SQL_KEYWORDS.has(tableName.toLowerCase()))
            continue;
        const modelName = toPascalCase(tableName);
        if (modelSchemasMap.has(modelName))
            continue; // Don't override decorator-based schemas
        const fields = [];
        const refFields = new Set();
        ...
        for (const colDef of columnDefs) {
            ...
            if (constraints.includes('DEFAULT')) {
                ...
                if (defMatch)
                    field.defaultValue = defMatch[1].toLowerCase();
            }
            fields.push(field);
        }
        if (fields.length > 0) {
 ...
 */
function extractDynamoDbTableInfo(objExpr, db, filePath) {
    let tableName = null;
...
function extractDynamoDbFieldsFromObject(objExpr, modelName, filePath) {
    if (!modelName)
        return;
    const fields = [];
    for (const prop of objExpr.properties) {
...
function toPascalCase(name) {
    ...
    if (/^[A-Z_]+$/.test(name) && name.length <= 6)
        return name;
    return name
        .split(/[_\-\s]+/)
