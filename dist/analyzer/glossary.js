...
function buildGlossary(input) {
    ...
    for (const [resource, data] of routeResources) {
        if (terms.has(normalize(resource)))
            continue; // already covered by entity
        upsert(terms, {
    ...
    for (const [fp] of input.files) {
        ...
        if (!className)
            continue;
        if (terms.has(normalize(className)))
            continue;
        upsert(terms, {
            term: className,
            kind: 'service',
    ...
    for (const route of input.routes) {
        ...
        for (const guard of route.guards) {
            ...
            for (const m of matches) {
                ...
                if (ROLE_TOKENS.has(stripped))
                    rolesFound.add(stripped);
            }
        }
    }
    ...
    const order = { entity: 0, route: 1, service: 2, role: 3, concept: 4 };
...
function composeEntityDefinition(name, fieldCount, fkRefs) {
...
function serviceClassName(filePath) {
